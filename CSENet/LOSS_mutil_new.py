...
class UUNetloss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        out, d1, d2, soble, weights = inputs
        ...
        # weights[i] σ2
        weight1 = 0.5 * torch.exp(-weights[0])
        weight2 = 0.5 * torch.exp(-weights[1])
        weight3 = 0.5 * torch.exp(-weights[2])
        weight4 = 0.5 * torch.exp(-weights[3])
        # weight5 = 0.2 / (weights[4] ** 2)


        l = torch.sqrt(weights[0] * weights[1] * weights[2] * weights[3])
        l = torch.log(l+1)

        # l5 = torch.log(1 + weights[4] ** 2)


        loss = loss1 * weight1 + loss2 * weight2 + loss3 * weight3 + loss4 * weight4 + l
               # loss5 * weight5 + l5
